...
//==============================================================================
  ...
  var fTmp = new CForcer();       // create a force-causing object
  //spring for two particals
  fTmp.forceType = F_SPRING;      // set it to spring
  fTmp.targFirst = 0;             // apply it to ALL particles:
  fTmp.targCount = 0;
  fTmp.K_springdamp = 1.0;
  fTmp.K_restLength = 0.5;
  fTmp.K_spring = 2.0;
  fTmp.e1 = 0;                    // apply it to the first partical
  fTmp.e2 = 1;                    // apply it to the second partical
...
// Create a vertex buffer object (VBO) in the graphics hardware: get its ID# 
  ...
  console.log(this.vboID+ 
    ' first bufferid for ps');
...
// (this function will simplify our too-complicated 'draw()' function)
  ...
  var j = 0;  // i==particle number; j==array index for i-th particle
  var xtmp, ytmp, ztmp;
  var dist2, radius2, normer, stretch, mag;
  //var e0to1;       // unit-length vector from spring endpoint e0 to e1.
  for(var i = 0; i < this.partCount; i += 1, j+= PART_MAXVAR) {
    s[j + PART_X_FTOT] = 0.0;
    s[j + PART_Y_FTOT] = 0.0;
    s[j + PART_Z_FTOT] = 0.0;
  }
  //GLdouble e0to1[3];         // unit-length vector from spring endpoint e0 to e1.
  e0to1 = new Float32Array(3);
...
//    console.log("fList[k].forceType:", fList[k].forceType);
    ...
    switch(fList[k].forceType) {    // what kind of force should we apply?
      ...
      case F_SPRING:
        ...
        var j1 = fList[k].e1*PART_MAXVAR;
        var j2 = fList[k].e2*PART_MAXVAR;

        e0to1[0] = s[j2 + PART_XPOS] - s[j1 + PART_XPOS];
        e0to1[1] = s[j2 + PART_YPOS] - s[j1 + PART_YPOS];
        e0to1[2] = s[j2 + PART_ZPOS] - s[j1 + PART_ZPOS];
        dist2 = e0to1[0]*e0to1[0] + e0to1[1]*e0to1[1] + e0to1[2]*e0to1[2];

        normer = Math.sqrt(dist2);
        ...
        s[j1 + PART_X_FTOT] += mag * e0to1[0];
        s[j1 + PART_Y_FTOT] += mag * e0to1[1];
        s[j1 + PART_Z_FTOT] += mag * e0to1[2];
        // equal and opposite force on particle e1.
        s[j2 + PART_X_FTOT] -= mag * e0to1[0];
        s[j2 + PART_Y_FTOT] -= mag * e0to1[1];
        s[j2 + PART_Z_FTOT] -= mag * e0to1[2];
        // find net velocity............................
        xtmp = s[j1 + PART_XVEL] - s[j2 + PART_XVEL];
        ytmp = s[j1 + PART_YVEL] - s[j2 + PART_YVEL];
        ztmp = s[j1 + PART_ZVEL] - s[j2 + PART_ZVEL];
        // find dot-product of net velocity with e0to1 unit vector:
        mag = xtmp * e0to1[0] + ytmp * e0to1[1] + ztmp * e0to1[2];
